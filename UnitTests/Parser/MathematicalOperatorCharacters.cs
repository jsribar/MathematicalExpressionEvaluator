...
namespace Parser
{
    ...
    public class MathematicalOperatorCharacters
    {
        ...
        public void ParseMethodReturnsExpressionForConstantPrecededByMinusSign()
        {
            ...
            Assert.AreEqual(-3, parser.Parse("−3").Interpret(new MathematicalExpressionEvaluator.Expressions.Context(5)), 1e-10);
            Assert.AreEqual(-6, parser.Parse("\u22126").Interpret(new MathematicalExpressionEvaluator.Expressions.Context(5)), 1e-10);
        }
        ...
        {
            var parser = new MathematicalExpressionEvaluator.Parser();
            Assert.AreEqual(2.0 / 3, parser.Parse("2 ÷ 3").Interpret(new MathematicalExpressionEvaluator.Expressions.Context(5)), 1e-10);
            Assert.AreEqual(3.0 / 2, parser.Parse("3 \u00F7 2").Interpret(new MathematicalExpressionEvaluator.Expressions.Context(5)), 1e-10);
        }
    }
}
